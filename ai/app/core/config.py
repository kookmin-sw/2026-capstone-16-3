...
ALERT_DISTANCE_M = float(os.getenv("ALERT_DISTANCE_M", "5.5"))
MAX_GUIDE_DISTANCE_M = float(os.getenv("MAX_GUIDE_DISTANCE_M", "8.0"))
NEAR_M = float(os.getenv("NEAR_M", "3.0"))
MID_M = float(os.getenv("MID_M", "5.5"))
...
VEHICLE_MIN_TRACK_AGE = int(os.getenv("VEHICLE_MIN_TRACK_AGE", "2"))

# Motion / tracking
MOTION_PIXEL_THRESHOLD = float(os.getenv("MOTION_PIXEL_THRESHOLD", "30.0")) # 실제 영상에서 정지/이동 케이스를 보고 튜닝해야 함!
TRACK_HISTORY_LEN = int(os.getenv("TRACK_HISTORY_LEN", "4"))

# Cooldown
DYNAMIC_OBJ_COOLDOWN_FRAMES = int(os.getenv("DYNAMIC_OBJ_COOLDOWN_FRAMES", "10"))
STATIC_OBJ_COOLDOWN_FRAMES = int(os.getenv("STATIC_OBJ_COOLDOWN_FRAMES", "20"))
SCENE_INFO_COOLDOWN_FRAMES = int(os.getenv("SCENE_INFO_COOLDOWN_FRAMES", "20"))
...
DEPTH_SCALE_FACTOR = float(os.getenv("DEPTH_SCALE_FACTOR", "0.6515"))  # 실측 후 조정
DEPTH_OFFSET_M = float(os.getenv("DEPTH_OFFSET_M", "-0.4844"))           # 선형 보정 b항